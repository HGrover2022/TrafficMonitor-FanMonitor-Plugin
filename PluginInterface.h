...
public:
    ...
    //预留的接口
    virtual void* OnItemInfo(ItemInfoType type, void* para1, void* para2) { return 0; }
...
public:
    ...
    enum DPIType
    {
        ...
        DPI_TASK_BAR    /**< 任务栏窗口的DPI */
    };

    /**
     * @brief   获取当前DPI值
     * @param   type 获取主窗口还是任务栏窗口的DPI
     * @return  DPI值
     */
    virtual int GetDPI(DPIType type) = 0;

    /**
     * @brief   根据id获取一个字符串
     * @detail  此函数返回主程序支持的多语言字符串，
     *  插件可以调用此函数来获取诸如“上传”、“下载”、“CPU”等这类通用的字符串。
     *  插件可以根据ITrafficMonitor::GetLanguageId()获取的语言ID来决定返回什么语言的字符串。
     * @param   id 字符串的id
     * @return  const wchar_t* 字符串
     */
    virtual const wchar_t* GetString(const wchar_t* id) = 0;
};
